...
export async function login(
  ...
  try {
    ...
    return {
      ...
      mustChangePassword:
        profile.role === "student" && profile.is_first_login === true,
    };
  } catch {
...
export async function requestPasswordReset(
  ...
  try {
    ...
    if (usersError) {
      console.error(
        "[requestPasswordReset] Falha ao listar usuários do auth:",
        usersError,
      );
    ...
    if (profileError) {
      console.error(
        `[requestPasswordReset] Falha ao buscar perfil do usuário ${user.id}:`,
        profileError,
      );
    }
    ...
    if (insertError) {
      console.error(
        "[requestPasswordReset] Falha ao inserir notificação:",
        insertError,
      );
    }
