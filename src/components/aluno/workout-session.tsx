...
export function WorkoutSession({
  ...
  const sessionStartedAt = useRef(
    savedSession?.startedAt ?? new Date().toISOString(),
  );
  ...
  const progress = useMemo(
    () =>
      exercises.length > 0
        ? (completedExerciseIds.length / exercises.length) * 100
        : 0,
    [completedExerciseIds.length, exercises.length],
  );

  const allCompletedHaveWeight = completedExerciseIds.every((id) =>
    isValidWeight(weights[id]),
  );
  ...
  function handleCompleteSet(exerciseId: string, setIndex: number) {
    ...
    if (!isLastSetOfExercise) {
      ...
      const nextExercise =
        exercises.find(
          (e, i) =>
            i > exerciseIndex && !newCompletedExerciseIds.includes(e.id),
  ...
  async function handleFinish() {
    const invalid = completedExerciseIds.find(
      (id) => !isValidWeight(weights[id]),
    );
    if (invalid) {
      setErrorId(invalid);
