...
import { getExerciseWeightHistory, type WeightHistoryEntry } from "@/app/actions"
...
export function ExerciseHistoryModal({ exerciseId, exerciseName, onClose }: ExerciseHistoryModalProps) {
      ...
      <div className="relative bg-zinc-900 border border-zinc-800 rounded-t-2xl md:rounded-2xl w-full md:max-w-md max-h-[80vh] overflow-hidden flex flex-col animate-in slide-in-from-bottom-4 md:zoom-in-95 duration-200">
        <div className="flex items-center justify-between px-4 py-3 border-b border-zinc-800">
          <div className="min-w-0">
            <p className="text-zinc-500 text-[9px] font-bold uppercase tracking-widest">Evolução de Carga</p>
            <h3 className="text-white font-black uppercase text-sm truncate">{exerciseName}</h3>
          </div>
        ...
        <div className="overflow-y-auto p-4">
          {loading ? (
            ...
            <p className="text-zinc-500 text-xs text-center py-10">
              Nenhum registro de carga ainda. Finalize um treino com o peso preenchido.
            </p>
          ) : (
            <div className="space-y-2">
              {history.map((entry, i) => {
                // history is newest-first, so the next item is the previous run.
                const { Icon, color } = trendFor(entry.weight, history[i + 1]?.weight)
                return (
                  <div
                    key={`${entry.date}-${i}`}
                  ...
                  >
                    ...
                    <span className="flex items-center gap-2">
                      <span className="text-white text-sm font-black">{entry.weight} kg</span>
